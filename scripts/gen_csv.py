...
for model_name in models:
    ...
    csv_data.append(
        [model_name, "Dynamic", "Static", "Dynamic - Static", "Static/Dynamic (%)"]
    )
    ...
    for metric in metrics:
        ...
        csv_data[r_idx][3] = str(
            round(model_data["dynamic"][metric] - model_data["static"][metric], 2)
        ).replace(".", ",")
        csv_data[r_idx][4] = str(
            round(model_data["static"][metric] / model_data["dynamic"][metric], 4)
        ).replace(".", ",")
        r_idx += 1
