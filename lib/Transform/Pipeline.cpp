...
void addReorderPOCPipeline(OpPassManager &pm) {
  pm.addPass(rarog::createReorderPOCPass());
}
...
void registerReorderPOCPipeline() {
  PassPipelineRegistration<>(
      ...
      "Proof of Concept for using Shuffle Graph to change"
      "relative order of instructions",
      addReorderPOCPipeline);
}
